...
```
PUT https://{domain}/oapi/v1/codeup/organizations/{organizationId}/repositories/{repositoryId}
```

### **Region版**

```
PUT https://{domain}/oapi/v1/codeup/repositories/{repositoryId}
```
...
| \\- | object | body | 否   |     |     |
| default\\_branch | string | body | 否   | 默认分支。 | master |
| description | string | body | 否   | 代码库描述，最长65535个字符。 | 描述内容 |
| name | string | body | 否   | 代码库名称。 | demo-repo |
| path | string | body | 否   | 代码库路径。 | demo-repo |
| visibility | string | body | 否   | 可见性：private（私有）、internal（组织内公开），可能的值：\\[private internal\\]。 | private |
...
| **参数**                   | **类型** | **描述**                                                     | **示例值**                                    |
| -------------------------- | -------- | ------------------------------------------------------------ | --------------------------------------------- |
| \\-                        | object   | PUT 更新代码库接口响应；字段与 GET 详情基本一致，但顶层 accessLevel 为权限数值（与 GET 的枚举字符串区分）。 |                                               |
| accessLevel                | integer  | 当前用户在该代码库上的权限等级，可能的值：\\[20 30 40\\]。   | 40                                            |
